...
let pzd = generate("HellMode", 20260621)
console.dir(pzd, {depth:null, color:true})

function printMinoShape(mino: MinoData): void {
    const cells = mino.cell;
    if (cells.length === 0) return;

    // 1. ミノ全体の最小・最大座標を求めて、幅と高さを計算する
    ...
    // 2. キャンバス（2次元配列）を半角スペースで初期化
    const canvas: string[][] = Array.from({ length: height }, () => 
        Array(width).fill(" ")
    );

    // 3. 最小座標からの相対位置（ローカル座標）にセルの文字を配置
    for (const c of cells) {
        const localX = c.x - minX;
        const localY = c.y - minY;
        canvas[localY][localX] = c.type;
    }
...
/**
 * PuzzleDataを受け取り、含まれるすべてのミノ（ピース）を表示するメイン関数
 */
export function printAllPiecesFromPuzzle(puzzle: PuzzleData): void {
    const minoDataList = puzzle[1]; // PuzzleDataの2番目の要素がmino_data

    minoDataList.forEach((mino, index) => {
